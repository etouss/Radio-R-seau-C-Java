...
void serialize()
{
  int fic = open(SER, O_WRONLY | O_CREAT | O_TRUNC, 0600);
  lockfile(fic, F_WRLCK);
  /* uint8_t board[SIZE][SIZE]; */
  int i = 0;
  int j = 0;
  for (i = 0; i < SIZE; i++)
  {
    for (j = 0; j < SIZE; j++)
    {
      uint8_t w = htons((i + j + board[i][j]) % 12);
      write(fic, &board[i][j], sizeof(uint8_t));
    }
    write(fic, "\n", strlen("\n"));
  }
  close(fic);
...
uint8_t ** deserialize(int fd)
{
  uint8_t nBoard[SIZE][SIZE];
  int i = 0;
  int j = 0;
  uint8_t r;
  char rc;
  for (i = 0; i < SIZE; i++)
  {
    for (j = 0; j < SIZE; j++)
    {
      read(fd,&r,sizeof(uint8_t));
      board[i][j]=(ntohs(r)-i-j+24)%12; 
    }
    read(fd,&rc,1);
  }
  close(fd);
  write_tty("deserial OK\n");
  return nBoard;
}
